...
// Bubble Sort algorithm (updated to enable smooth animation)
void bubbleSort(std::vector<int>& bars, sf::RenderWindow& window, sf::Color barColor,
    int sortingSpeed) {
    for (size_t i = 0; i < bars.size(); i++) {
        bool swapped = false;
        ...
        if (!swapped) {
            // If no swaps were made, the array is sorted early
            break;
        }
    }
}
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // Sort the bars
        bubbleSort(bars, window, barColor, sortingSpeed);
    }
